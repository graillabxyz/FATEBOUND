    ...
    <div className={`battle-screen phase-${view.phase.toLowerCase()}`}>
      ...
      >
        ...
        <div className="opponent-copy">
          <div>
            ...
            <RankBadge small label={presentation?.ranks?.[1] ?? "Stone III"} />
          </div>
          <h2>{enemy.name}</h2>
      ...
      <div className="universal-actions">
        <button data-card-target="guard" onClick={() => assign("guard")}>
          ...
          {plan.assignments.filter((a) => a.target === "guard").length > 0 && (
            <b>{plan.assignments.filter((a) => a.target === "guard").length}</b>
          )}
        </button>
